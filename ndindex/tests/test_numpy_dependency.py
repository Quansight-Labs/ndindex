...
def _test_numpy_dependency_ndindex():
...
def _test_numpy_dependency_iter_indices():
    assert 'numpy' not in sys.modules
...
def _test_numpy_dependency_indices():
    for index_type, args in [
        ...
        assert 'numpy' not in sys.modules, index_type
...
def _test_numpy_dependency_ChunkSize():
    assert 'numpy' not in sys.modules
...
from ndindex.tests.test_numpy_dependency import {func}

{func}()'''], check=True, capture_output=True, encoding='utf-8')
