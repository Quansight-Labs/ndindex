...
[^tuple-ellipsis-footnote]: There is one important distinction between the
    ...
    footnotes [^view-scalar-footnote] and [^view-0d-footnote] for two
...
[^integer-scalar-footnote]: In fact, if the integer array index itself has
    ...
    `int` with the same value.[^view-0d-footnote] So it's a true
    generalization. In ndindex,
...
[^view-0d-footnote]: There is one difference between `a[0]` and
    `a[asarray(0)]`. The latter is considered an advanced index, so it does
    not create a [view](views-vs-copies).[^view-scalar-footnote]
...
multiaxis index) that contains at least one array index.[^view-0d-footnote]
